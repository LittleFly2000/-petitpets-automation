...
def claude_chat(prompt: str, max_tokens: int = 3000) -> str:
    """使用 Claude API（支持自定义 BASE_URL 的代理）"""
...
def get_sheet():
    ...
    return gc.open_by_url(os.environ["GOOGLE_SHEET_URL"])
    ...
    try:
        ...
        write_tab(sh, "自动_Shopify产品", [
            ...
            [
                ...
                p.get("title"),
                p.get("product_type"),
                p.get("variants", [{}])[0].get("price", ""),
                p.get("variants", [{}])[0].get("inventory_quantity", ""),
                p.get("status"),
                p.get("created_at", "")[:10],
            ] for p in products
        ])

        write_tab(sh, "自动_Shopify订单7天", [
        ...
        write_tab(sh, "自动_TikTok爆款", [
            "来源", "查询", "账号", "描述", "播放量", "点赞", "分享", "评论", "BGM", "链接",
        ], [
            [
                r["source"], r["query"], r["author"], r["description"],
                r["plays"], r["likes"], r["shares"], r["comments"],
                r["music"], r["url"],
            ] for r in tt_results[:50]
        ])

        write_tab(sh, "自动_YouTube爆款", [
            "关键词", "标题", "频道", "观看", "点赞", "发布", "链接",
        ], [
            [
                r["keyword"], r["title"], r["channel"],
                r["views"], r["likes"], r["published"][:10], r["url"],
            ] for r in yt_results[:50]
        ])

        write_tab(sh, "自动_本周Claude分析", [
            "时间", "分析全文",
        ], [
