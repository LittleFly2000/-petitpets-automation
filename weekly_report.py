...
def _parse_service_account_json(raw: str) -> dict:
    """宽容 parser：处理常见粘贴错误（多余空白 / 粘贴了两遍 / 前后有垃圾字符）"""
    raw = raw.strip()
    # 先直接试
    try:
        return json.loads(raw)
    except json.JSONDecodeError as e:
        log(f"   ⚠️ 直接 JSON 解析失败 ({e})，尝试修复...")
        log(f"   JSON 长度: {len(raw)} 字符, 开头: {raw[:20]!r}, 结尾: {raw[-20:]!r}")

    # 兜底 1：提取第一个完整的 JSON 对象（处理「粘贴两遍」的情况）
    depth = 0
    start = -1
        ...
        if c == "{":
            ...
            if depth == 0 and start >= 0:
                try:
                    ...
                    log(f"   ✅ 成功从第 {start}-{i} 字符提取出有效 JSON")
                    return parsed
                except Exception:
                    continue

    raise ValueError(
        "GOOGLE_SERVICE_ACCOUNT_JSON 无法解析。"
        "请检查 Secret 里是不是粘贴了两次 / 有多余字符 / 格式被破坏。"
    )


def get_sheet():
    creds_json = _parse_service_account_json(os.environ["GOOGLE_SERVICE_ACCOUNT_JSON"])
